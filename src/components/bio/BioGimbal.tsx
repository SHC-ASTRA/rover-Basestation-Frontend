    ...
    return <>
        <div className="horizontal-split">
            <div className="vertical-split container">
                ...
                <div className="vertical-split">
                    <h3>
                        Latitude:&nbsp;
                        <span>{coreFeedback ? coreFeedback.data.gps_lat.toPrecision(7) : "no data"}&nbsp;&deg;N</span>
                    </h3>
                    <h3>
                        Longitude:&nbsp;
                        <span>{coreFeedback ? coreFeedback.data.gps_long.toPrecision(7) : "no data"}&nbsp;&deg;E</span>
                    </h3>
                </div>
            </div>
            <div className="vertical-split container">
        ...
        <div className="horizontal-split">
            <div className="vertical-split container">
                <h2>Temperature</h2>
                <h3>
                    {bioFeedback ? bioFeedback.data.drill_temp.toPrecision(1) : "no data"}&nbsp;&deg;C
                </h3>
            </div>
            <div className="vertical-split container">
                <h2>Humidity</h2>
                <h3>{bioFeedback ? bioFeedback.data.drill_humidity.toPrecision(1) : "no data"}&nbsp;%</h3>
            </div>
        </div>
    </>
}
...
function PTZControls() {
    ...
    function setTargetZoom(newZoom: { zoom_level: number }) {
        const clampedZoomLevel = Math.max(0, Math.min(6, newZoom.zoom_level));
